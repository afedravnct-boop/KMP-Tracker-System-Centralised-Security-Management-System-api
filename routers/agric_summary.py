...
def get_agric_summaries(db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    return [{
        "id": r.id, "sn": r.sn or idx + 1, "region": r.region, "station": r.station, "date": r.date,
        "agric_crime_report": r.agric_crime_report, "number_count": r.number_count,
        "recoveries": r.recoveries, "status": r.status, "last_updated_by": r.last_updated_by
...
def create_agric_summary(data: dict, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    try:
        new_record = Model(
            ...
            status=str(data.get("status", "UNDER INVESTIGATION")).upper(),
