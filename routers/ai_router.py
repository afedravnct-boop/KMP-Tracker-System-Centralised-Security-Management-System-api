...
async def process_tactical_query(
    ...
    try:
        ...
        system_rules = (
            ...
            "- DATA INGESTION VS. OPSEC RESTRICTIONS: When instructing users on how to upload, import, or update the Nominal Roll, you MUST explicitly state that the uploaded file must contain ALL columns, including sensitive administrative data. Direct them to fill out the complete standard template, which includes: IPPS Number, National ID (NIN), Tax Identification Number (TIN), Bank Account Number (acc_no), Bank Branch, and Contact/Phone Number. Clarify to the user: 'While the AI Command Console strictly masks and restricts querying this sensitive PII for OPSEC reasons, the raw database upload MUST contain the complete and unredacted dossier to maintain HR ledger integrity.' Never tell a user to remove sensitive columns from their upload files.\n"
            ...
            "- SYSTEM USER GUIDE: Covers authentication, the sign-up workflow (requiring valid NIN starting with CM/CF and a mandatory photo), Crime Registry with Agri-Crimes filtering, Disruptive OPS statistics, Success Stories, Establishments, Nominal Roll tracking, Documents & Reports, and Master Database exports.\n"
