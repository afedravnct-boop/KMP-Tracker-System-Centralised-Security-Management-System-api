...
def verify_command_clearance(current_user, target_user, action_type="GRANT_PRIVILEGE"):
    """
    Validates hierarchical command clearance based on UPF structure:
    Police HQ -> KMP HQ -> Regional HQ (Regional Police Commander & Deputy) -> Divisions/Stations.
    Super Admin (God Mode) possesses absolute clearance without limitations.
    """
    ...
    target_pos = (target_user.position or "").upper()
    target_station = (target_user.station or "").upper()
    target_region = (target_user.region or "").upper()

    # 🟢 1. SUPER ADMIN (GOD MODE): Absolute, unhindered global access
    if curr_role == "SUPER_ADMIN" or "GOD-MODE" in curr_pos or "SUPER_ADMIN" in curr_role:
        return True

    # 🟢 2. PREVENT SELF-CLEARING
    ...
    # 🟢 3. POLICE HQ & KMP HQ COMMAND LEVEL (KMP Commander, Deputy, CID, CI, Departmental Heads)
    is_hq_command = (
        curr_station in ["POLICE HEADQUARTERS", "KMP HEADQUARTERS"] or
        "KMP COMMANDER" in curr_pos or
        "DEPUTY KMP COMMANDER" in curr_pos or
        "CID COMMANDER" in curr_pos or
        "CI COMMANDER" in curr_pos or
        "DIRECTOR" in curr_pos or
        "IGP" in curr_pos or
        "DIGP" in curr_pos
    )
    if is_hq_command:
    ...
    # 🟢 4. REGIONAL POLICE COMMANDERS & DEPUTY REGIONAL POLICE COMMANDERS
    is_regional_commander = (
        curr_role == "RPC" or 
        "RPC" in curr_pos or 
        "REGIONAL POLICE COMMANDER" in curr_pos or 
        "DEPUTY REGIONAL POLICE COMMANDER" in curr_pos
    )
    if is_regional_commander:
        # Can clear personnel within their own region, but cannot modify higher HQ or Super Admin
        if target_region == curr_region and target_role not in ["SUPER_ADMIN"] and "REGIONAL POLICE COMMANDER" not in target_pos:
            return True
        raise HTTPException(
            status_code=403, 
            detail=f"Clearance Denied: Regional Leadership for {curr_region} can only authorize or clear personnel within their assigned region."
        )

    # 🟢 5. DIVISIONAL / STATION COMMANDERS
    is_local_commander = "DIV" in curr_pos or "COMMANDER" in curr_pos or "DPC" in curr_pos or curr_role in ["ADMIN", "STATION_ADMIN"]
    if is_local_commander:
        if target_station == curr_station and target_role not in ["SUPER_ADMIN", "RPC"] and "REGIONAL POLICE COMMANDER" not in target_pos:
            return True
        raise HTTPException(
            status_code=403, 
            detail="Clearance Denied: Station/Division commanders can only clear subordinates within their local station."
        )

    # 🟢 6. DEFAULT RESTRICTION FOR MASTER DOWNLOADS & SUBORDINATES
    if action_type == "DOWNLOAD_MASTER":
        if curr_role in ["SUPER_ADMIN", "ADMIN"] or is_hq_command or is_regional_commander:
            return True
        raise HTTPException(
            status_code=403, 
            detail="Clearance Denied: Station personnel must be cleared by regional or command authority to access master reports."
        )

    raise HTTPException(status_code=403, detail="Clearance Denied: Insufficient command rank and jurisdiction for this clearance action.")
...
def create_report(data: dict, db: Session = Depends(get_db), current_user: models.Users = Depends(get_current_user)):
    try:
        ...
        if incoming_sd_ref or incoming_narrative:
            # Fetch existing station reports to check against
            ...
            for r in existing_reports:
                ...
                # Rule 1: Reject if Reference Number is an exact match in the same station
                ...
                # Rule 2: Reject if Narrative is completely identical and not blank
                ...
                # Rule 3: Check for similar offences with highly overlapping narrative text (e.g. over 85% string similarity or identical core description)
                if incoming_offence and existing_offence and incoming_offence == existing_offence:
                    if incoming_narrative and existing_narrative:
                        # Simple keyword/substring overlap check for similar narratives under same offence
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                row_dict['nin'] = nin_val if nin_val != 'NONE' else None
                ...
                clean_row = {k: v for k, v in row_dict.items() if k in valid_keys and v is not None and str(v).strip().lower() not in ["", "none", "nan", "nat"]}
                ...
                existing_record = db.query(models.NominalRoll).filter(fnum_attr == fnum_val).first()
...
def download_official_template(template_type: str, current_user: models.Users = Depends(get_current_user)):
    ...
    filename_map = {
        "weekly-report": "KMP_Weekly_Report_Template.docx",
        "assignment": "KMP_Task_Assignment_Template.docx"
    }
    
    filename = filename_map.get(template_type)
    if not filename:
        raise HTTPException(status_code=404, detail="Requested template not found.")
